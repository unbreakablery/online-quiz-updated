<?php
    ...
    if($_FILES["source_file"]["size"] > 0) {
        ...
        if ( $xlsx = SimpleXLSX::parse($filename) ) {
            ...
            $table_header = ['#', 'Quiz Code', 'Count of Questions', 'Quiz Type', 'Limit Time', 'Quiz Kind', 'Page'];
            $table_data = [];
            foreach ( $xlsx->rows() as $k => $r ) {
                ...
                $page = strtolower(trim(mysqli_real_escape_string($connect, $r[19])));
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    $query .= "(`quiz_code`, `quiz_type`, `limit_time`, `quiz_kind`, `page`)";
                    $query .= " VALUES ";
                    $query .= "('$quiz_code', '$quiz_type', $limit_time, '$quiz_kind', '$page')";
                ...
                $query .= "(`quiz_id`, `que_id`, `que_type`, `que_text`, `ans_1`, `ans_2`, `ans_3`, `ans_4`, `ans_5`, `ans_6`, `ans_7`, `ans_8`, `cor_ans`, `cor_fb`, `inc_fb`, `points`, `video`)";
                $query .= " VALUES ";
                $query .= "(";
                $query .= $quiz_id . ", ";
                $query .= $r[1] . ", ";
                ...
                $query .= "'" . trim(mysqli_real_escape_string($connect, $r[20])) . "'";
                $query .= ")";
                $result = mysqli_query($connect, $query);
            }
