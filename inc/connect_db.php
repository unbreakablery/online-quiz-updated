<?php
	...
	if ($env == "dev") {
		...
		// $connect = mysqli_connect('localhost','main_user','Ge7qC]*s{Rp=') or die('Database connection failed.');
		$connect = mysqli_connect('localhost','root','') or die('Database connection failed.');
		mysqli_select_db($connect, 'emedica-quiz-fsjt');
	} else if ($env == "staging") {
	...
	function getScore($que_type, $cor_ans, $answers, $points = 4) {
		...
		if ($que_type == "SEQ") {
			...
			$score = ($answers == 0  || $answers == '') ? 0 : ($points - abs(ord($cor_ans) - 65 + 1 - $answers));
		}
	...
	function getPageDesc($page) {
		...
		switch($page) {
			case 'fsjt':
				$desc = "We have separated the SJT questions by type (as in the real exam).";
				$desc .= " Once you have worked through these, try the full SJT mock at the bottom of the page.";
				break;
			case 'ratings':
				$desc = "This service has only ratings with 5 options.";
				$desc .= " And timed mock has about 140 ratings questions.";
				break;
			case 's-f-sjt':
				$desc = "This is sample for Foundation SJT Quiz.";
				$desc .= " A quiz will have each 2 questions per ratings(5 options), selection, and ranking.";
				$desc .= " Once you have worked through these, try to login and then the real exam.";
				break;
			case 's-msra':
				$desc = "This is sample for MSRA Quiz.";
				$desc .= " A quiz will have each 2 questions per selection(select 5 from 8), and ranking.";
				$desc .= " Once you have worked through these, try to login and then the real exam.";
				break;
			default:
				$desc = '';
	...
	function getQueIDs($quiz_id, $q_type_order = 'ASC') {
		global $connect;
		$query = "
					SELECT 
						temp.id
					FROM 
						...
						WHERE
							q.quiz_id = $quiz_id
						ORDER BY q.que_id) AS temp
					ORDER BY temp.que_type $q_type_order, temp.que_id ASC
                ";
        $result = mysqli_query($connect, $query);
        $que_ids = array();
	...
	function getQuizzes($page = null) {
		global $connect;
		$where = isset($page) ? " WHERE `q1`.`page` = '" . $page . "' " : "";
		$query = "
					SELECT
						`q1`.`id`,
						`q1`.`quiz_code`,
	...
	function saveQuiz($data) {
		...
		$query = "
					INSERT INTO `quizzes` 
						(`quiz_code`, `quiz_type`, `limit_time`, `quiz_kind`, `page`) 
					VALUES 
						('$quiz_code', '$quiz_type', $limit_time, '$quiz_kind', '$page')
